...
public:
    ...
    friend fraction operator+(fraction const&a,fraction const&b);//fraction c=a+b
    friend fraction operator-(fraction const&a,fraction const&b);//c=a-b
    friend fraction operator*(fraction const&a,fraction const&b);//c=a*b
    friend fraction operator/(fraction const&a,fraction const&b);//c=a/b
    fraction operator-()const;//b=-a
    fraction reciprocal()const;//倒数操作,返回一个新的分数
    ...
    friend bool operator>=(fraction const&a,fraction const&b);//if(a>=b)
    friend bool operator<=(fraction const&a,fraction const&b);//if(a<=b)
    friend istream& operator>>(istream& in,fraction& com);//overload >>
    friend ostream& operator<<(ostream& os,const fraction& com);//overload <<
